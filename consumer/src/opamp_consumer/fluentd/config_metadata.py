...
"""Fluentd config metadata extraction helpers."""
...
from opamp_consumer.abstract_client import resolve_service_instance_id_template
...
def extract_fluentd_config_metadata(
    config_path: str | pathlib.Path | None,
) -> ConfigMetadata:
    """Extract Fluentd metadata comments and raw config text.

    Args:
        config_path: Fluentd config file path.
    ...
    for raw_line in config_text.splitlines():
        ...
        if key == CONFIG_METADATA_KEY_SERVICE_INSTANCE_ID:
            value = str(resolve_service_instance_id_template(value) or "").strip()
        if value:
    ...
    return ConfigMetadata(
        ...
        config_type=metadata_values.get(CONFIG_METADATA_KEY_CONFIG_TYPE, "fluentd"),
