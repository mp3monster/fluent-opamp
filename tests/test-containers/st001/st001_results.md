...
# ST-001 Creation and Verification Results

Generated: 2026-08-10

## Outcome

The ST-001 container artefacts were created under `tests/test-containers/st001`.
...
```bash
OPAMP_SOCKET_E2E_SUMMARY_PATH=/mnt/d/dev/opamp/dist/test-reports/st001/local-socket/socket_e2e_summary.json \
...
- `dist/test-reports/st001/local-socket/socket_e2e_summary.json`
...
| `scripts/run_st001.sh` | Builds, starts, verifies, captures logs, and tears down one or both profiles. |
| `scripts/verify_st001.py` | Verifies provider registration, channel, `/tool/otelAgents`, and disconnect evidence. |
...
## Reconciliation Against ST-001

| ST-001 requirement | Coverage |
|---|---|
...
| Evidence retained | `dist/test-reports/st001/<profile>/` receives JSON, logs, summary, and `results.md`. |
...
```bash
bash -n tests/test-containers/st001/scripts/prepare-source.sh
bash -n tests/test-containers/st001/scripts/provider-entrypoint.sh
bash -n tests/test-containers/st001/scripts/consumer-entrypoint.sh
bash -n tests/test-containers/st001/scripts/run_st001.sh
python3 -m py_compile tests/test-containers/st001/scripts/verify_st001.py
python3 -m ruff check tests/test-containers/st001/scripts/verify_st001.py
python3 -m json.tool tests/test-containers/st001/config/provider/opamp-provider.json
python3 -m json.tool tests/test-containers/st001/config/consumer/socket/opamp-consumer.json
python3 -m json.tool tests/test-containers/st001/config/consumer/socket/simulator-responses.json
python3 -m json.tool tests/test-containers/st001/config/consumer/http/opamp-consumer.json
python3 -m json.tool tests/test-containers/st001/config/consumer/http/simulator-responses.json
```
...
That will produce the full container evidence under `dist/test-reports/st001/socket`
and `dist/test-reports/st001/http`.
