...
if [[ "${use_local}" == "true" && -d "${local_source}" ]]; then
  if [[ -d "${local_source}/.git" ]]; then
    echo "[st002] retrieving source from mounted checkout ${local_source}" >&2
  ...
  if [[ -d "${local_source}/provider" && -d "${local_source}/consumer" ]]; then
    echo "[st002] copying mounted source tree ${local_source}" >&2
    find "${destination}" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
...
if [[ ! -d "${destination}/.git" ]]; then
  echo "[st002] cloning ${source_repo} into ${destination}" >&2
  find "${destination}" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
  ...
  echo "[st002] refreshing existing checkout ${destination}" >&2
